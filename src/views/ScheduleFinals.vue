<script setup>
    ...
    onBeforeMount(async () => {
        ...
        {
            urlSchedule += `/${route.params[props.urlSchedule.split('$')[i].split('/')[0]]}/` + `${props.urlSchedule.split('$')[i].split('/')[1]}`
        }
        urlJson.value = (await axios.get(urlSchedule)).data;

        if (props.type == 'st-fin')
        {
            const urlName = props.urlName.split('$')[0] + route.params[props.urlName.split('$')[props.urlName.split('$').length - 1].split('/')[0]]
            group.value = Object.keys((await axios.get(urlName)).data)[0];
        }
        else if (props.type == 'pr-fin')
        {
            const urlName = props.urlName.split('$')[0] + route.params[props.urlName.split('$')[1]]
        ...
        {
            weeks.value = finalsDataRefactoring.func(urlJson.value, props.sort, timesJson)
        }
    })
</script>
...
<template>
    ...
    <Suspense timeout="0">
        <template #default>
            <MainFrame v-if="weeks" :type="type" :groupName="group" :info="[]" :weeks="weeks" :errorMessage="errorMessage"></MainFrame>
        </template>
        <template #fallback>
            <Spinner />
        </template>
